...
var di = function (fs, config, mode, dirName) {
    ...
    var checkConfig = () => {
        ...
        for (var i = 0; i < modes.length; i++) {
            ...
            for (var j = 0; j < deps.length; j++) {
                ...
                if (typeof config[mode][dependency] !== 'string') {
                    if (config[mode][dependency].require === undefined) {
                        throw new CustomError('DI_ERROR_CONFIG_DEPENDENCY', 'Wrong configuration for dependency ' + dependency + ' in ' + mode);
                    }

                    // if list of required arguments specified need to check it
                    if (config[mode][dependency].args !== undefined) {
                        if (typeof config[mode][dependency].args !== 'string' && !Array.isArray(config[mode][dependency].args)) {
                            throw new CustomError('DI_ERROR_CONFIG_DEPENDENCY_ARGS', 'Wrong arguments for dependency ' + dependency + ' in ' + mode);
                        }
                    }
                }
            }
     ...
     */
    this.get = (dependency) => {
        if (['DI', 'di'].indexOf(dependency) > -1) {
            return this;
        } else if (dependencies[dependency] !== undefined) {
            ...
            if (typeof config[curMode][dependency] === 'string') {
                dep = require(getPath.bind(this)(config[curMode][dependency]));
            } else {
                var args = config[curMode][dependency].args !== undefined ? prepareArguments.bind(this)(config[curMode][dependency].args) : [];
